...
TEST_P(Dedupv1dExecutionTest, WriteCrashRead) {
    EXPECT_LOGGING(dedupv1::test::ERROR).Times(0, 10);
    EXPECT_LOGGING(dedupv1::test::WARN).Times(0, 10);
    EXPECT_LOGGING(dedupv1::test::WARN).Matches("Still 184467440.* open new blocks").Never();
    ...
    ASSERT_TRUE(ds->dedup_system()->chunk_store()->Flush(NO_EC));
    ...
    INFO("Restart");
    dedupv1::StartContext start_context2(dedupv1::StartContext::NON_CREATE, dedupv1::StartContext::DIRTY, dedupv1::StartContext::NO_FORCE);
    start_context2.set_crashed(true);
    ds = Create(GetParam());
    ASSERT_TRUE(ds);
    ASSERT_TRUE(ds->Start(start_context2)) << "Cannot start application";
    ASSERT_TRUE(ds->Run());
    ...
    std::list<std::string>::iterator li_w1 = fp_write1.begin();
    std::list<std::string>::iterator li_w2 = fp_write2.begin();
    std::list<std::string>::iterator li_r1 = fp_read2.begin();

    uint64_t offset = 0;
    for (; li_w2 != fp_write2.end() && li_w1 != fp_write1.end() && li_r1 != fp_read2.end(); li_w2++, li_w1++, li_r1++, offset += (64 * 1024)) {
        EXPECT_TRUE(*li_w1 == *li_r1 || *li_w2 == *li_r1) <<
        "Either old or new write but not in between: " <<
        "offset " << offset <<
        ", write 1 " << *li_w1 <<
        ", write 2 " << *li_w2 <<
        ", read 1 " << *li_r1;
