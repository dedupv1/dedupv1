...
import os
from scons_check import run, CheckModprobe, CheckExecutable, SconsVersion, Config, Execute, IsMac, GenerateCheckLibraryVersionFunction
import platform
...
debug_cflags = ['-ggdb', '-g']
release_cflags = ['-O3', '-DNDEBUG', '-DNVALGRIND', '-g'] # In release we also include the DebugStrings, but we will separate them later
...
# because log4cxx is known to crash in some situations.
...
# TODO (dmeister): What is the difference between scons with default targets and --no-tests
...
# if the flag --research is set, patent protected stuff is compiled (is only allowed to be used in the University context)
AddOption("--research", dest="research", action="store_true", default=False)
...
if not default_dedupv1_config:
    default_dedupv1_config = os.path.normpath(os.path.join(GetOption('prefix'), "etc/dedupv1/dedupv1.conf"))
...
CheckMHDVersion = GenerateCheckLibraryVersionFunction("microhttpd", "microhttpd.h", "MHD_get_version")
...
if build_mode != "release":
    ...
    if conf.CheckExecutable("distcc") and not GetOption("no_distcc"):
        distcc_hosts = filter(lambda line: len(line) > 0, Execute("distcc --show-hosts", redirect_stderr=False).split("\n"))
...
if not conf.CheckCXXHeader("tbb/concurrent_unordered_map.h"):
    ...
    Exit(1)
# re2 library. For strange reasons this switch between Mac und real unixes is necessary
if IsMac():
...
config.BuildConfigHeader(["base/include/base/config.h"], ["base/include/base/config.h.in"])
config.BuildConfigModule(["dedupv1_util/src/lib/config.py"], ["dedupv1_util/src/lib/config.py.in"])
if build_test:
    dedupv1_core_test_prog = SConscript("core/unit_test/SConscript", variant_dir = join("build", build_mode, "dedupv1_core_test"))
    test_targets.append(dedupv1_core_test_prog)
    all_targets.append(dedupv1_core_test_prog)

    dedupv1_base_test_prog = SConscript("base/unit_test/SConscript", variant_dir = join("build", build_mode, "dedupv1_base_test")) 
    test_targets.append(dedupv1_base_test_prog)
    all_targets.append(dedupv1_base_test_prog)

    dedupv1d_test_prog = SConscript("dedupv1d/unit_test/SConscript", variant_dir = join("build", build_mode, "dedupv1d_test"))
    all_targets.append(dedupv1d_test_prog)
    test_targets.append(dedupv1d_test_prog)

    dedupv1_contrib_test_prog = SConscript("contrib/unit_test/SConscript", variant_dir = join("build", build_mode, "dedupv1_contrib_test")) 
    test_targets.append(dedupv1_contrib_test_prog)
    all_targets.append(dedupv1_contrib_test_prog)

if build_dedupv1d:
    dedupv1d_prog = SConscript("dedupv1d/SConscript", variant_dir = join("build", build_mode, "dedupv1d"))
...
if build_contrib:
    ...
    dedupv1_starter_prog = SConscript("contrib/dedupv1_starter/SConscript", variant_dir = join("build", build_mode, "dedupv1_starter"))
