...
class Dedupv1Test:
    def __init__(self, run, sys, config = "/opt/dedupv1/etc/dedupv1/dedupv1.conf", user = None):
        self.run = run
        self.config = config
        self.user = user
        self.sys = sys
    ...
    def start(self, *args, **kwargs):
        self.insert_extra_params(kwargs)
        r = self.run("/opt/dedupv1/bin/dedupv1_adm start", "--config=%s" % sh_escape(self.config), *args, **kwargs)
