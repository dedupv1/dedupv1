...
class Dedupv1DynamicConfigSystemTest(Dedupv1BaseSystemTest):
    ...
    def test_user_normal(self):
        ...
        self.dedupv1.targets("add tid=4 name=iqn.2005-05.de.cim:systemtest")
        self.assertExitcode(0)
        ...
        self.dedupv1.users("target add name=admin5 target=iqn.2005-05.de.cim:systemtest")
        self.assertExitcode(0)
        
        users_output = self.dedupv1.users("show")
        self.assertExitcode(0)
        self.assertTrue(users_output.find("iqn.2005-05.de.cim:systemtest") >= 0)
        
        targets_output = self.dedupv1.targets("show")
        self.assertExitcode(0)
        self.assertTrue(targets_output.find("admin5") >= 0)

        self.dedupv1.users("target remove name=admin5 target=iqn.2005-05.de.cim:systemtest")
        self.assertExitcode(0)
    ...
    def test_change_user(self):
        ...
        users_in_target = self.scst.get_user_in_target("iqn.2005-05.de.cim:systemtest")
...
class Dedupv1ISCSISystemTest(Dedupv1BaseSystemTest):
    ...
    def test_lun_missing(self):
    	...
    	def check_available_lun(luns):
    	    self.open_iscsi.discover(server="127.0.0.1")
    	    self.assertExitcode(0)
    	    self.open_iscsi.connect(server="127.0.0.1", name="iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk")
    	    self.assertExitcode(0)
    	    try:  
                sleep(5)
                devices = os.listdir("/dev/disk/by-path")
                ...
                for lun in luns:
                    self.assertTrue("ip-127.0.0.1:3260-iscsi-iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk-lun-%s" % (lun) in devices)
            finally:
                  self.open_iscsi.disconnect(server="127.0.0.1", name="iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk")
                  self.assertExitcode(0)    
            
        self.start_default_system()
        self.sys.rm_scst_local()

        self.dedupv1.targets("add tid=3 name=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk")

        self.dedupv1.volumes("attach id=4 device-name=backup_cim logical-size=500G target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk:0") 
        self.assertExitcode(0)
        self.dedupv1.volumes("attach id=5 device-name=backup_bil logical-size=500G target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk:1") 
        self.assertExitcode(0)

        check_available_lun(["0", "1"])

        self.dedupv1.volumes("attach id=6 device-name=dynamic logical-size=500G target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk:2") 
        self.assertExitcode(0)
        self.dedupv1.volumes("attach id=7 device-name=dynamic1 logical-size=500G target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk:3") 
        self.assertExitcode(0)

        check_available_lun(["0", "1", "2", "3"])

        print self.scst.get_devices_in_group("Default_iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk")

        self.dedupv1.volumes("target remove id=6 target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk --verbose --debug")
        self.assertExitcode(0)

        print self.scst.get_devices_in_group("Default_iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk")

        self.dedupv1.volumes("target remove id=7 target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk")
        self.assertExitcode(0)
        ...
        self.dedupv1.volumes("attach id=8 device-name=backup_kal logical-size=500G target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk:2") 
        self.assertExitcode(0)
        ...
        self.dedupv1.volumes("target remove id=8 target=iqn.2007-08.neu.cim:dedupe-cim.cim.neu.disk")
        self.assertExitcode(0)
...
class Dedupv1DataSystemTest(Dedupv1BaseSystemTest):
    ...
    def test_urandom_kill_after_volume_detach(self):
        ...
        if (process_name):
            ...
            # If start still fails because dedupe can not attach to port 9001 and this message
